...
```
show terraform website
explain that terraform can be run as enterprise, tfc, and cli -> for purposes of learning only covering cli (workflow is the same)
go to downloads page
explain os/architectures/other options
copy download link (because we will use linux)
open terminal
curl link, unzip, move
```