...
<Video id="MJIcYDfCACE"/>

```
slideware
what are providers?
how does terraform use/interact with providers?
how do providers interact with interfaces/services/apis
```
...
<Video id="xO1_FhFfgbQ"/>

```
where do I get providers?
show terraform registry
show how to find a provider (docker)
```
...
<Video id="eX51ziztMQc"/>

<Task id="install_provider">
  Add the `vault` provider to the current Terraform configuration and initialize the workspace.
</Task>

```
show how to add a provider to your code
show how to version a provider
run terraform init
show how the lock file works
```