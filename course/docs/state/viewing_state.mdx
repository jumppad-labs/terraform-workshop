...
where is state stored? local vs remote backend -> local for learning purposes
```
...
<Video id="CRBjDXbHdik"/>

<Task id="viewing_state">
...
explain output
run terraform show with -json + jq
explain output
```